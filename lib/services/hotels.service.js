...
export async function getHotelComplete(id) {
  try {
    ...
    const [
      ...
      supabase.from('hotel_faqs').select('*').eq('hotel_id', id).order('id', { ascending: true }),
    ]);
    ...
    const faqs = faqsResult && !faqsResult.error ? (faqsResult.data || []) : [];
...
/**
 * Insert FAQs for a hotel (create flow).
 * @param {number} hotelId
 * @param {Array<{ question_en: string, question_ar?: string, answer_en: string, answer_ar?: string }>} faqs
 * @returns {Promise<{ data: Array|null, error: string|null }>}
 */
export async function saveHotelFaqs(hotelId, faqs) {
  try {
    if (!supabase) {
      return { data: null, error: 'Supabase not configured' };
    }
    ...
    const rows = faqs
      .filter((f) => f && (f.question_en || f.answer_en))
      .map((f) => ({
        hotel_id: hotelId,
    ...
    const { data, error } = await supabase
      ...
      .select();
    if (error) {
      console.error('Supabase error saving FAQs:', error);
      throw error;
    }
    return { data: data || [], error: null };
  } catch (error) {
    console.error('Error saving FAQs:', error.message || error);
    return { data: null, error: error.message || 'Failed to save FAQs' };
  }
}

/**
 * Upsert FAQs for a hotel and delete removed ones (edit flow).
 * @param {number} hotelId
 * @param {Array<{ id?: number, question_en: string, question_ar?: string, answer_en: string, answer_ar?: string }>} faqs
 * @returns {Promise<{ data: Array|null, error: string|null }>}
 */
export async function updateHotelFaqs(hotelId, faqs) {
  try {
    if (!supabase) return { data: null, error: 'Supabase not configured' };

    // Fetch existing FAQ IDs
    const { data: existing } = await supabase
      .from('hotel_faqs')
      .select('id')
      .eq('hotel_id', hotelId);

    const cleaned = (faqs || []).filter((f) => f && (f.question_en || f.answer_en));
    const currentIds = cleaned.map((f) => f.id).filter(Boolean);
    ...
    cleaned.forEach((f) => {
      ...
      if (f.id && Number.isInteger(f.id) && f.id > 0) {
        row.id = f.id;
    ...
    const allFaqs = [...inserted, ...upserted];

    // Fetch and return all FAQs
    ...
    console.error('Error updating FAQs:', error.message || error);
    return { data: null, error: error.message || 'Failed to update FAQs' };
  }
}
 ...
 * @param {number} hotelId
 * @param {Array<{ average_rating: number, total_reviews: number }>} reviews
 * @returns {Promise<{ data: Array|null, error: string|null }>}
 */
export async function saveHotelReviewAggregates(hotelId, reviews) {
  try {
    ...
    // Only one allowed because UNIQUE(hotel_id)
    const review = reviews[0];

    const row = {
      hotel_id: hotelId,
      average_rating: Number(parseFloat(review.average_rating) || 0),
      total_reviews: Math.max(0, parseInt(review.total_reviews, 10) || 0),
...
/**
 * Upsert review aggregates by (hotel_id) 
 * @param {number} hotelId
 * @param {Array<{ id?: number, average_rating: number, total_reviews: number }>} reviews
 * @returns {Promise<{ data: Array|null, error: string|null }>}
 */
export async function updateHotelReviewAggregates(hotelId, reviews) {
  try {
    ...
    if (!reviews || reviews.length === 0) {
      // Optional: delete review if empty
      await supabase
        .from('review_aggregates')
        .delete()
        .eq('hotel_id', hotelId);
...
/**
 * Full hotel update (main record + amenities, rooms, images, FAQs, review aggregates)
 * @param {number} id - Hotel ID
 * @param {Object} hotelData - Main hotel fields
 * @param {Object} relatedData - { amenities?, rooms?, imageUrls?, faqs?, reviewAggregates? }
 * @returns {Promise<{ data: Object|null, error: string|null }>}
 */
export async function updateHotelComplete(id, hotelData, relatedData = {}) {
  try {
    ...
    if (relatedData.faqs !== undefined) {
      updatePromises.push(updateHotelFaqs(id, relatedData.faqs));
    }
